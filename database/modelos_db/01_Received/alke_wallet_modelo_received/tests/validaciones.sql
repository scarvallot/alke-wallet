-- =====================================================
-- Validaciones — AlkeWallet Modelo Received
...
-- Catálogo de monedas (en este modelo Monedas no está vinculada a Usuarios)
SELECT
    currency_id,
    currency_name,
    currency_symbol
...
LEFT JOIN `Usuarios` AS u ON t.sender_user_id = u.user_id
...
FROM `Usuarios` u
LEFT JOIN `Transacciones` t ON u.user_id = t.receiver_user_id
...
INNER JOIN `Usuarios` emisor ON t.sender_user_id = emisor.user_id
INNER JOIN `Usuarios` receptor ON t.receiver_user_id = receptor.user_id
...
SELECT
    ...
    t.transaction_date
FROM `Transacciones` t
INNER JOIN `Usuarios` u ON t.sender_user_id = u.user_id
...
SELECT
    ...
    u.correo_electronico
FROM `Usuarios` u
WHERE u.user_id IN (SELECT DISTINCT sender_user_id FROM `Transacciones`);
