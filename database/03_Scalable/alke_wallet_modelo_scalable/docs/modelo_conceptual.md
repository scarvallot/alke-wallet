...
El modelo **Scalable** representa el esquema final de AlkeWallet, alcanzando la **Tercera Forma Normal (3FN)**. Su principal innovación es la introducción de la entidad `Accounts`, que permite a los usuarios tener **múltiples saldos en diferentes monedas** y desacopla el saldo de la tabla `Users`. Este modelo está diseñado para entornos productivos que requieran flexibilidad, integridad y auditoría avanzada.
...
Representa a cada persona registrada en el sistema. **Ya no almacena saldo**; ese dato se traslada a `Accounts`.
...
### 2.3. `Accounts` (Cuenta) — **Nueva entidad**

Representa el saldo de un **usuario** en una **moneda** específica. Es una entidad débil que depende de `Users` y `Currencies`.

| Atributo            | Tipo              | Restricciones                                           | Descripción                                  |
| :------------------ | :---------------- | :------------------------------------------------------ | :-------------------------------------------- |
| `account_id`      | `INT`           | **PK**, `AUTO_INCREMENT`                        | Identificador único de la cuenta.            |
| `user_id`         | `INT`           | `NOT NULL`, **FK** → `Users(user_id)`         | Usuario propietario de la cuenta.             |
...
El siguiente diagrama Mermaid representa visualmente las cuatro entidades y sus relaciones, incluyendo la nueva tabla `Accounts`:
...
- ✅ Soporte multi‑moneda (tabla `Accounts`).
...
- **Borrado lógico**: No se implementa `is_active` en `Users`; se recomienda como extensión.
