...
SELECT 
    ...
    t.`transaction_date`
FROM `Transactions` t
INNER JOIN `Accounts` a_sender   ON t.`sender_account_id`  = a_sender.`account_id`
INNER JOIN `Users` u_sender      ON a_sender.`user_id`      = u_sender.`user_id`
INNER JOIN `Accounts` a_receiver ON t.`receive_account_id`  = a_receiver.`account_id`
INNER JOIN `Users` u_receiver    ON a_receiver.`user_id`    = u_receiver.`user_id`
INNER JOIN `Currencies` c_sender   ON a_sender.`currency_id`   = c_sender.`currency_id`
INNER JOIN `Currencies` c_receiver ON a_receiver.`currency_id` = c_receiver.`currency_id`
...
INSERT INTO `Transactions` (importe, sender_account_id, receive_account_id)
VALUES (50000, 3, 7);

-- 4. Verificar que todo quedó correcto antes de confirmar
SELECT account_id, current_balance FROM `Accounts` WHERE account_id IN (3, 7);
...
SELECT account_id, current_balance FROM `Accounts` WHERE account_id = 3;
...
SELECT * FROM `Transactions` WHERE sender_account_id = 3 AND importe = 20000;
