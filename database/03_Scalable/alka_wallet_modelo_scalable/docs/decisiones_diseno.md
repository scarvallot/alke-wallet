...
| **Relación User ↔ Currency** | Indirecta (vía`Transaction`)     | Directa (vía`Account`)                                                         | Cada cuenta asocia un usuario con una moneda de forma explícita.             |
...
| **Cuenta predeterminada**      | No existía                         | `is_default` en `Account`                                                     | Permite identificar la cuenta principal del usuario (lógica de aplicación). |
...
### 3.1. Tabla `Account` (Cuenta por moneda)
...
- **Solución**: Se crea la tabla `Account`, que actúa como una **entidad débil** entre `User` y `Currency`. Cada registro representa el saldo de un usuario en una moneda específica.
- **Ventajas**:
  - Soporte nativo para multi‑moneda.
  - Escalabilidad: se pueden agregar nuevas monedas sin modificar la estructura.
  - Normalización 3FN (el saldo ya no es un atributo de `User`).
...
- **Cambio**: En lugar de `sender_user_id` y `receiver_user_id`, la tabla `Transaction` ahora usa `sender_account_id` y `receiver_account_id`.
...
- **Impacto**: La consulta *"¿Cuánto dinero ha enviado el usuario X?"* ahora requiere unir `Transaction` con `Account` para obtener el `user_id`.
...
- **Excepción**: `ON DELETE CASCADE` se mantiene en `Account` → `Transaction`? **No**, también se usa `RESTRICT` para proteger el historial.
...
| :-------------- | :--------------------------------------------------------------------------- | :--------------------------------------------------------------------------- |
| `Account`     | `CHECK (balance >= 0)`                                                     | Evita saldos negativos (no se puede deber dinero al sistema).                |
| `Transaction` | `CHECK (importe > 0)`                                                      | El monto debe ser positivo (no se permiten transferencias de 0 o negativas). |
| `Transaction` | `CHECK (sender_account_id <> receiver_account_id)`                         | Evita que una cuenta se transfiera dinero a sí misma.                       |
| `User`        | `CHECK (email REGEXP '^[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\\.[A-Za-z]{2,}$')` | (Opcional) Valida formato de correo.                                         |

### 3.5. `is_default` en `Account`
...
| :----------------------------------- | :-------------------------------------------------------- |
| **Soporte multi‑divisa**      | Tabla`Account` (saldo por moneda).                      |
| **Sin validaciones `CHECK`** | Restricciones de negocio en`Account` y `Transaction`. |
...
- **Borrado lógico**: No se implementa `is_active` en `User`; la eliminación física está restringida pero no se ofrece una alternativa de desactivación.

---

## 7. Conclusión

El modelo **Scalable** representa la culminación de la evolución del esquema AlkaWallet, alcanzando la **Tercera Forma Normal (3FN)** y proporcionando una base sólida para un sistema financiero real. Las decisiones de diseño priorizan:
