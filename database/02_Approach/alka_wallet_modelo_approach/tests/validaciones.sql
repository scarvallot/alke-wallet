-- =====================================================
-- Validaciones — AlkaWallet Modelo Approach
...
-- 1.1 Verificar que no haya emails duplicados en User
...
-- 2.3 Campos obligatorios en Transaction
...
-- 3.1 Transacciones con sender_user_id que no existe en User
SELECT t.*
FROM `Transaction` t
LEFT JOIN `User` u ON t.sender_user_id = u.user_id
WHERE u.user_id IS NULL;

-- 3.2 Transacciones con receiver_user_id que no existe en User
SELECT t.*
FROM `Transaction` t
LEFT JOIN `User` u ON t.receiver_user_id = u.user_id
WHERE u.user_id IS NULL;

-- 3.3 Transacciones con currency_id que no existe en Currency
SELECT t.*
FROM `Transaction` t
...
SELECT * FROM `User` WHERE current_balance < 0;
...
FROM `User` u
LEFT JOIN `Transaction` t ON u.user_id = t.sender_user_id OR u.user_id = t.receiver_user_id
...
FROM `User` u
JOIN `Transaction` t ON u.user_id = t.receiver_user_id
GROUP BY u.user_id, u.username
...
-- SELECT COUNT(*) FROM `Transaction` WHERE sender_user_id = 1 OR receiver_user_id = 1;
