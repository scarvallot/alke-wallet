# AlkaWallet — Modelo Approach (Mejorado)

Modelo de base de datos **mejorado** a partir del modelo inicial recibido (`Received`).
Incluye correcciones de tipos de datos, restricciones de integridad, vinculación de la entidad `Currency` con `Transaction`, y optimizaciones de rendimiento.
...
| :------------------------ | :------------------------------------- | :---------------------------------------------------------------------------------------------------------------------------------------------- |
| **`User`**        | Usuarios del monedero virtual          | `user_id` (PK), `username`, `email` (UNIQUE), `password`, `current_balance`                                                           |
| **`Currency`**    | Catálogo de divisas                   | `currency_id` (PK), `currency_name` (UNIQUE), `currency_symbol` (UNIQUE)                                                                  |
| **`Transaction`** | Movimientos financieros entre usuarios | `transaction_id` (PK), `importe`, `transaction_date` (DATETIME), `sender_user_id` (FK), `receiver_user_id` (FK), `currency_id` (FK) |
...
> El diagrama completo en Mermaid está disponible en [`diagrams/alka_wallet_er.mmd`](diagrams/alka_wallet_er.mmd).
...
| :------------------------------- | :---------------------------------------------------------------------------------- | :----------------------------------------------------------------- |
| **Soporte multi‑divisa**  | El usuario solo tiene un saldo global (no puede tener saldos en distintas monedas). | Extraer`current_balance` a una tabla `Account` (UserCurrency). |
